...
def format_progress(event: str, message: str, data: dict) -> None:
    ...
    if event == "phase_change":
        ...
        print(f"{Colors.DIM}[CHECKPOINT]{Colors.ENDC} Saved to {data.get('checkpoint_file')}")
...
async def run_conclave(args: argparse.Namespace) -> None:
    ...
    from src.infrastructure.adapters.external.crewai_adapter import create_crewai_adapter
    ...
    if args.resume:
        ...
        session_name = args.session or f"conclave-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    motion_title = args.motion or "Should AI systems be granted limited autonomous decision-making authority?"
    ...
    # Estimate time
    total_turns = len(archon_profiles) * config.max_debate_rounds + len(archon_profiles)  # debate + voting
    ...
    print(f"{Colors.DIM}({total_turns} total turns: {config.max_debate_rounds} debate rounds + 1 voting round){Colors.ENDC}")
    ...
    try:
        ...
        motion = await conclave.propose_motion(
        ...
        # Phase 8: Vote
        result = await conclave.conduct_vote()
        ...
        print(f"  Archons present: {summary['present_archons']}/{summary['total_archons']}")
        print(f"  Motions: {summary['motions_count']} ({summary['passed_motions']} passed, {summary['failed_motions']} failed)")
        ...
        print(f"  Resume with: python scripts/run_conclave.py --resume {checkpoint_path}")
